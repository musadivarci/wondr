...
export function TopicCard({ topic, index, isDraggable, onDragStart, onDragOver, onDrop, onDragEnd, onStudy, onEdit, onArchive, isArchived }: TopicCardProps) {
  return <article className="topic-card" id={topic.id} draggable={isDraggable} onDragStart={() => onDragStart(topic.id)} onDragOver={(event) => onDragOver(event, topic.id)} onDrop={() => onDrop(topic.id)} onDragEnd={onDragEnd}>
    ...
    <div className="topic-actions"><button className="study-button" type="button" onClick={() => onStudy(topic.id)}>Çalış <ArrowIcon/></button><button className="edit-button" type="button" onClick={() => onEdit(topic.id)}>Düzenle</button><button className="archive-button" type="button" onClick={() => onArchive(topic.id)}>{isArchived ? 'Arşivden çıkar' : 'Arşivle'}</button></div>
  </article>
}
